...
// import { aw } from 'dist/assets/index-YI5OslZn';
...
//console.log('saleOrderStore.saleorder.docdate ', saleOrderStore.saleorder.docdate);
...
//let docdate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.docdate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
//let nextDuedate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.nextduedate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
//let deliveryDate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.deliverydate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
...
onMounted(async () => {
    ...
    let objDocno = props.docno.toString().split('@@');
    let docno = objDocno[0]; // Get the first part of the docno
    let dbame = objDocno[1]; // Get the second part of the docno

    saleOrderStore.saleorder.docno = docno;
    saleOrderStore.saleorder.dbname = dbame; // Set the database name
    ...
    await saleOrderStore.fetchSaleOrderByDocno(docno);

    //console.log('saleOrderStore.saleorder.docdate ', saleOrderStore.saleorder.docdate);

    // docdate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.docdate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
    // nextDuedate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.nextduedate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
    // deliveryDate = shallowRef(adapter.parseISO(saleOrderStore.saleorder.deliverydate).toISOString().slice(0, 10)); // Format to YYYY-MM-DD
...
const handleSaveData = async () => {
  ...
  let result  = await saleOrderStore.editOrder();
  ...
  if(result){
    if (result=="true") {
      ...
      //await saleOrderStore.changeNextDueDate(); // change next due date

      //alert("บันทึกข้อมูลสั่งขายเรียบร้อยแล้ว");
      split_error.value = "แก้ไขข้อมูลสั่งขายเรียบร้อยแล้ว";
