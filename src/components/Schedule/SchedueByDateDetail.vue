<template>
    ...
    <v-card-title class="d-flex align-center pe-2">
      ...
      <v-row>
        <v-col cols="12" sm="8" class="d-flex align-center">
              <v-icon icon="mdi-cart-outline"></v-icon> &nbsp; {{PageLabel}} {{schedueStore.selectedThaiDate }}
            </v-col>
         <v-col>
            <v-text-field
              v-model="search"
  ...
  <v-col cols="12" class="d-flex justify-end my-2">
    <v-chip color="primary" style="font-weight: bold; font-size: 1.1rem;">
      ...
      {{
        (
          hideBydateTable
        ? schedueStore.appointmentItemsUpsaleByDate
        : schedueStore.appointmentItemsByDate
        )
          .filter(item => item.rowid !== null && item.rowid !== undefined)
          .length
      }}
    </v-chip>
  </v-col>
    ...
    <v-chip color="success" style="font-weight: bold; font-size: 1.1rem;">
      ...
      {{
        ...
        : schedueStore.appointmentItemsByDate
          ...
          .filter(item => item.due_status === 'โทรแล้ว')
          .length
      }}
    </v-chip>
  </v-col>
  ...
  <v-col cols="12" class="d-flex justify-end my-2">
    <v-chip color="error" style="font-weight: bold; font-size: 1.1rem;">
      ...
      {{
        ...
        : schedueStore.appointmentItemsByDate
          ...
          .filter(item => item.due_status === 'ยังไม่ได้โทร')
          .length
      }}
    </v-chip>
  </v-col>
    ...
    <v-data-table
      ...
      :items="schedueStore.appointmentItemsByDate"
      :items-per-page="500"
      :headers="headers"
    ...
    >
      ...
      <template v-slot:item.duedate="{ item }">
        <v-chip color="pink" class="text-end" font-weight="bold">
           {{  item.duedate }}
        </v-chip>
      </template>

      <template v-slot:item.teldate="{ item }">
            <v-chip color="blue" class="text-end">
            {{ item.due_status === 'ยังไม่ได้โทร' ? '' : item.teldate }}
            </v-chip>
      </template>

       <template v-slot:item.due_status="{ item }">
        <v-chip :color="item.due_status === 'ยังไม่ได้โทร' ? 'red' : 'green'" class="text-end" font-weight="bold">
           {{ item.due_status }}
        </v-chip>
      </template>
    ...
    <v-data-table
      ...
      v-model:search="search"
      :items="schedueStore.appointmentItemsUpsaleByDate"
      :items-per-page="25"
...
const schedueStore = useScheduleStore();
...
//<router-link :to="`/salecom/${item.rowid +'@@bydate'}`"
  ...
  if(xtype=='bydate'){
      schedueStore.fetchSaleComDetailByDate(rowid);
      PageLabel.value = 'นัดหมาย วันที่';
      hideBydateTable.value = false;
      hideUpsaleTable.value = true;
  }else{
      schedueStore.fetchSaleComUpsaleByDate(rowid);
